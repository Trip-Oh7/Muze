...
class NotesApp(MDApp):
    ...
    def add_note_to_grid(self, note):
        ...
        preview_label = MDLabel(
            ...
            halign="left",  # Align text to the left
            valign="middle",  # Center vertically in its space
            font_style="Body1",  # Use a smaller font style for preview
        )
    ...
    def open_note_dialog(self):
        ...
        title_box = MDBoxLayout(
            orientation="horizontal", 
            size_hint_y=None, 
            height="30dp",  # Set minimal height
        )
        ...
        self.notes_field = TextInput(
            hint_text="Notes", 
            multiline=True, 
            size_hint_y=0.75,  # Height for the notes field
            padding=[10, 10],  # Padding inside the text area
        )
        ...
        self.dialog = MDDialog(
            ...
            pos_hint={'center_x': 0.5, 'center_y': 0.9},  # Adjust center_y as needed
    ...
    def toggle_favorite(self, *args):
        ...
        if self.current_note is None:
            # Initialize a new note if it doesn't exist
            self.current_note = {
                "title": self.title_field.text,
                "url": self.url_field.text,
                "notes": self.notes_field.text,
                            "category": self.category_field.text,  # Make sure category is included

                "favorite": False
            }

        # Toggle the favorite status
        if self.current_note.get("favorite", False):
            # If it's already a favorite, remove from favorites
            self.current_note["favorite"] = False
            self.favorite_button.icon = "star-outline"  # Set to unfilled star
        else:
            # If it's not a favorite, add to favorites
            self.current_note["favorite"] = True
            self.favorite_button.icon = "star"  # Set to filled star
    ...
    def on_search_text(self, instance, value):
        """Filter notes based on the search query, including categories."""
        filtered_notes = [
            note for note in self.notes 
            if value.lower() in note["title"].lower() or value.lower() in note.get("category", "").lower()
        ]
